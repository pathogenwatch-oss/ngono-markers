...
def find_frameshift(query: str, sbjct: str) -> bool:
    ...
    if len(insert_matches) != 0:
        return True
    deletion_matches = [
        deletion for deletion in indel_finder.findall(sbjct) if len(deletion) % 3 != 0
    ]
    return len(deletion_matches) != 0
...
def only_exact_matches(hits: Iterable[Match]) -> List[Match]:
